...
class _SejarawanState extends State<Sejarawan> {
  ...
  Future<void> _getsejarah() async {
    try {
      http.Response res = await http.get(Uri.parse('http://192.168.0.100/kebudayaan_server/getSejarawan.php'));
 ...
 Future<void> _deleteSejarawan(String id) async {
  try {
    http.Response res = await http.post(
      Uri.parse('http://192.168.0.100/kebudayaan_server/deleteSejarawan.php'),
  ...
  void _filtersejarah(String keyword) {
    ...
    setState(() {
      _filteredSejarah = _sejarahList.where((sejarah) =>
          sejarah.nama.toLowerCase().contains(keyword)
      ).toList();
    });
  }
              ...
              children: [
                Padding(
                  ...
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(20),
                    child: Image.network(
                      'http://192.168.0.100/kebudayaan_server/gambar_sejarawan/${data.foto}',
                      width: 122,
                      height: 118,
                      fit: BoxFit.cover,
                    ),
